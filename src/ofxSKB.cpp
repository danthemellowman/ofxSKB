...
void ofxSKB::draw() {
    ...
    if (kbShow) {
        ...
        ofDrawRectangle(x, y, kbWidth, kbHeight);
        ...
        for(unsigned int i=0; i<keys.size(); i++) {
            ...
            ofFill();
            ofDrawRectangle(xpos, ypos, keys[i].width, keys[i].height);
        ...
        if (keyTest(keys[i], ofGetAppPtr()->mouseX, ofGetAppPtr()->mouseY)) {
            ...
            switch(keys[i].key[tmp]) {
                ...
                case OFXSKB_KEY_CAPS:
                    break;
                case OF_KEY_DEL:
                    arg.key = OF_KEY_BACKSPACE;
                    ofNotifyEvent(ofEvents().keyPressed, arg);
                    break;
                case OF_KEY_RETURN:
                    arg.key = OF_KEY_RETURN;
                    ofNotifyEvent(ofEvents().keyPressed, arg);
                    break;
                case OFXSKB_KEY_MOVE:
                    moveKeyboard = true;
